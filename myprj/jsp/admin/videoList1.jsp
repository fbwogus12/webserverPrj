...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/css/admin/board/adminBoardListCSS/videoList1.css?ver=1.08">
<link
...
<script
	src="${pageContext.request.contextPath}/js/admin/board/adminBoardListJS/videoList1.js?ver=1.02"></script>
<title>카드 게시판 리스트 관리</title>
</head>
					...
					<form action="/admin/board/list?board=${param.board}" method="post">
						...
						<div class="board-list">
							<table class="board-table">
								<caption class="blind">게시판 리스트 관리</caption>
									...
									<c:forEach var="ul" items="${urgList}">
										...
										<tr class="list_style1 urg_style2" style="position: relative;">
											...
											<td class="title_style"><a id="font_urg"
												...
												<span class="cmt_style1">${ul.cmtCount == 0?""
                                                        :[ul.cmtCount]}</span>
											</td>
											...
											<td class="regdate_style"><fmt:parseDate
													...
													value="${dateFmt }" pattern="yyyy년 MM월 dd일" /></td>
										...
										<div class="photo_style" style="position: relative;">
											<div class="img_style">
												<c:if test="${empty b.files}">
													<a
														href="/admin/board/detail?board=${param.board}&id=${b.id}&p=${empty param.p?1:param.p}&f=${param.f}&q=${param.q}"><img
														class="img_style" src="" alt="파일이 없음. 삭제 요망"></a>
												</c:if>
												<a
													href="/admin/board/detail?board=${param.board}&id=${b.id}&p=${empty param.p?1:param.p}&f=${param.f}&q=${param.q}">
													<c:set var="id" value="${fn:split(b.files, ',')[0]}" /> <c:if
														test="${fn:split(id, '.')[1] eq 'yt'}">
														<img
															src="https://i.ytimg.com/vi/${fn:split(id, '.')[0]}/hqdefault.jpg">
													</c:if>
												</a>
											</div>
											...
											<span class="regdate_style"><fmt:parseDate
												...
												class="hit_style">조회수 : ${b.hit}</span> <label
												style="color: black; font-weight: bolder">공개<input
												type="checkbox" name="open_id" value="${b.id }" ${open }
												value="true"></label>
											<%--pub==true였으면 open에는 checked값이 들어 있으므로 체크됨.--%>
											<label style="color: red; font-weight: bolder">삭제<input
												type="checkbox" name="del_id" value="${b.id}"></label> <label
												style="color: blue; font-weight: bolder">강조<input
												type="checkbox" name="urg_id" value="${b.id }" ${urg }
												value="true"></label>
										</div>
									</c:forEach>
						...
						<div class="admin-button remote-button">
							<h2>리모콘</h2>
							<c:set var="ids" value="" />
							<c:forEach var="b" items="${listAll}">
								<c:set var="ids" value="${ids} ${b.id}" />
							...
							<input type="hidden" name="ids" value="${ids}"> <input
								type="submit" class="admin_btn" name="btn" value="공개하기">
							<input type="submit" class="admin_btn" name="btn" value="강조하기">
							<a class="admin_btn" href="/admin/board/reg?board=${param.board}">글쓰기</a>
							<input type="submit" class="admin_btn" name="btn" value="삭제하기">
						</div>
					</form>
				</div>
			</div>
