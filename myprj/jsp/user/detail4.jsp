...
<link rel="stylesheet" type="text/css"
	href="${pageContext.request.contextPath}/css/board/boardDetailCSS/detail4.css?ver=1.00">
<link
...
<script
	src="${pageContext.request.contextPath}/js/board/boardDetailJS/detail4.js?ver=1.01"></script>
								...
								<tbody>
										...
										<td style="text-align: left;">
											<ul type="square">
												<c:forTokens var="fileName" items="${b.files }" delims=","
													varStatus="st">
													<c:if
														test="${fn:split(fn:toLowerCase(fileName),'.')[1] eq 'jpg' || fn:split(fn:toLowerCase(fileName),'.')[1] eq 'png' ||  fn:split(fn:toLowerCase(fileName),'.')[1] eq 'gif' || fn:split(fn:toLowerCase(fileName),'.')[1] eq 'bmp'}">
									...
									<tr class="content-style">
										...
										<td><c:forTokens var="fileName" items="${b.files }"
												...
												pattern="yyyy-MM-dd HH:mm:ss" /><fmt:formatDate
												value="${dateFmt }" pattern="yyyyMMdd" />/${fileName}"
																		alt="${b.files}" />
							...
							<table class="move-table">
								...
								<tbody>
									<tr>
										...
										<c:if test="${next != 'null' }">
											<td class=""><a
												href="/board/detail?board=${param.board}&id=${next.id }&p=${param.p}&f=${param.f}&q=${param.q}">${next.title}</a></td>
										</c:if>
									</tr>
									<tr>
										<th>이전 글</th>
										...
										<c:if test="${prev != 'null' }">
											<td class=""><a
												href="/board/detail?board=${param.board}&id=${prev.id }&p=${param.p}&f=${param.f}&q=${param.q}">${prev.title}</a></td>
										</c:if>
									</tr>
								</tbody>
							</table>
